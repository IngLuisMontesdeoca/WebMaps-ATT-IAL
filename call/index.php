...
function notifySmsReception($correlator, $message) {
    ...
    $_path = '/var/www/dev/ial/call/';
    require_once $_path . 'Parlayx/configWS.php';
    $logger = new logger('/var/log/nxt/ial/dev/ws/SCReception/');
    ...
    $textoAlta = $message['message'];
    
    $plan = ( $textoAlta == 'Alta') ? 1 : 2;
    ...
    $userType = $Base->getUserTypeByPtn($ptn);
    $logger->_saveLog("OP|H=|Client=" . $utilities->getRealIP() . "-------------------------------------------------------|notifySCReception");
    $logger->_saveLog(date("d/m/Y H:i:s", time()) . ": " . $datos."|Prov={$textoAlta}");

    if ($idHandset == 0) {
        $logger->_saveLog("BD|H={$ptn}|El equipo no existe en BD|notifySmsReception");
        //Guardar PTN en BD
        $logger->_saveLog("BD|H={$ptn}|Guardando equipo en BD en BD(plan = {$plan})|notifySmsReception");
        $resSaveH = $parlayXProcess->saveHandsetSC($ptn, $plan);
        if ($resSaveH[1]) {
            $logger->_saveLog("BD|H={$ptn}|ID={$resSaveH[0]}|Equipo guardado correctamente en BD(plan = {$plan})|notifySmsReception");
            $idHandset = $resSaveH[0];
            if( $plan == 2){
                //Ejecutar cobro por evento(PayAsYouGo)
                $response = $parlayXProcess->aplyCharge($ptn, $idHandset,$userType);
            }
        } else {
        ...
        if ($tipoContrato == 2) {
            ...
            if( $plan == 2){
                ...
                $logger->_saveLog("BD|H={$ptn}|Actualizando tipo de contrato a Ilimitado|notifySmsReception");
                //Cambiar a contraro ilimitado
                if( $Base->updateTipoContratoByHandset($idHandset, 1, $parlayXProcess->paymentGetNextCutDate())){
                    $logger->_saveLog("BD|H={$ptn}|Tipo de contrato actualizado a Ilimitado Correctamente|notifySmsReception");
                }else{
                    $logger->_saveLog("ERR!!!|BD|H={$ptn}|Actualizando tipo de contrato a Ilimitado|notifySmsReception");
                }
            }
        } else {
            $logger->_saveLog("BD|H={$ptn}|Contrato ilimitado|notifySmsReception");
            if( $plan == 2){
                //Cambiar a cobro por evento(PayAsYouGo)
                $logger->_saveLog("BD|H={$ptn}|Actualizando tipo de contrato a Eventual|notifySmsReception");
                //Cambiar a contraro ilimitado
                if( $Base->updateTipoContratoByHandset($idHandset, 2, '')){
                    $logger->_saveLog("BD|H={$ptn}|Tipo de contrato actualizado a Eventual|notifySmsReception");
                }else{
                    $logger->_saveLog("ERR!!!|BD|H={$ptn}|Actualizando tipo de contrato a Eventual|notifySmsReception");
                }
                //Ejecutar cobro por evento(PayAsYouGo)
                $response = $parlayXProcess->aplyCharge($ptn, $idHandset,$userType);
            }
