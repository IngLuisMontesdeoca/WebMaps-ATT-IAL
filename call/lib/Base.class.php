...
class Base {
    ...
    function message($idHandset = 0, $message = "") {
        $query = "INSERT INTO dat_mensajes(n_handset_id, c_mensaje_tipo, c_mensaje_mensaje) VALUES(" . $idHandset . ", '2', '" . $message . "');";
        return mysqli_query($this->conn, $query);
    }
    ...
    /*******************************************************************/
    function saveHandset($ptn, $tipoContrato) {
        $this->_querySQL = "INSERT INTO dat_handset(c_handset_ptn, 
					  ...
					  VALUES('{$ptn}',NOW(), '', '', 1, 2, 1, {$tipoContrato}, 3, 2, 3);";
    ...
    function saveEstatusPayment($idHandset,$fechaCorte) {
        $this->_querySQL = "INSERT INTO dat_estatuspayment(d_estatuspayment_fechaproximo, 
					  ...
					  n_handset_id,
                                          n_estatus_id) 
					  VALUES('{$fechaCorte}', NOW(), 11, {$idHandset}, 3);";
        return mysqli_query($this->conn, $this->_querySQL);
    }
    ...
    function updateTipoContratoByHandset($idHandset, $tipoContrato, $fechaProximo = ''){
        $fechaRegistro = ($tipoContrato == 1) ? "d_handset_fecharegistro = NOW()," : "";
        $this->_querySQL = "UPDATE 	dat_handset "
                        . " SET	 	{$fechaRegistro}n_tipocontrato_id = {$tipoContrato}"
                        . " WHERE  	n_handset_id      = {$idHandset}";
        if( mysqli_query($this->conn, $this->_querySQL) ){
            $fechaProximo = "d_estatuspayment_fechaproximo = ".($tipoContrato == 1) ? "'{$fechaProximo}'," : "'0000-00-00 00:00:00'";
