<?php
    ...
    *   @description    Clase Abstracta que añade las funcionalida-	 *
	*					des CRUD básicas de ADOdb.					 *
	*																 *
	*	notes			Se incluye la funcionalidad de Paquetes, pa- *
    *                   trón Singleton para instancias de Objetos de *
    *                   Conexión a Bases de Datos, constructor y des-*
    *                   tructor para clases hijas y desparametriza-  *
    *                   ción del método save()                       *
	*																 *
    *****************************************************************/
  ...
  {//---------------------------------------------------------------------------------->>> Class Base
	...
	/*
	 * public 	Object
	 * desc		Objeto de Conexión a Base de Datos ADOdb
	 */
		public $baseDb = null;

	/*
	 * protected String
	 * desc		 Tabla con la que se trabajará
	 */
		protected $_baseTbl = '';
	...
	/*
	 * private 	Object
	 * desc		Contendrá la descripción de la tabla
	 */
		private $_meta = null;
     ...
     * desc     Instancia de ADObd para patrón Singleton
     */        
    ...
    */
	 ...
	 *			debug->Especifica si se realizará el debug de la consulta de definiciones
	 ...
	 */
		...
		{//<<-------------------------------------------------------------- Method setAndConnect
            ...
            //Patrón Singleton para Objetos de Conexión a Bases de Datos
	 ...
	 * @access	protected
	 * @param	tbl->Tabla con la que se trabajará
	 ...
	 */
        ...
        {//<<-------------------------------------------------------------- Method setPk
            ...
            /*
             * Si la llave primaria es un número y es indistinta de 0,
	...
	/*
	 * Obtiene las propiedades de la clase extraídas de la tabla y las devuelve en un objeto
	 * @access	private
	 */
	...
	/*
	 * Establece nuevas propiedades públicas a partir de un array dado. 
	 ...
	 * @access	private
	 * @params	orderby->Campo por el que se ordenará la consulta. Llave primaria por default
	 * @methods	this::_execute(), this::next()
	 */
		private function _load( $orderby = '' )
		{//<<-------------------------------------------------------------- Method _load

			//Sólo se efectuará la consulta si existe llave primaria
	...
	/*
	 * Obtiene todos los registros de la tabla con la que se está trabajando ordenados por la llave primaria de la misma
	 * @access	public
	 * @params	orderby->Campo por el que se ordenará la consulta. Llave primaria por default
	 ...
	 */
		...
		{//<<-------------------------------------------------------------- Method _loadIf
			...
			//Sólo se hará la consulta si el parametro es un arreglo
			if( !is_array($array) ) {
				return false;
			}
			...
			foreach( $array as $index => $value ) {
				...
				//Sólo consideramos las propiedades establecidas en la clase
				if( !isset($this->$index) ) {
					continue;
				}
	...
	/*
	 * Obtiene le número de registros encontrados en una consulta SELECT
	 * @access	public
	...
	/*
	 * Realiza la sentencia UPDATE o INSERT dependiendo de si se estableció una llave primaria o no
	 * @access	public
	 ...
	 */
		...
		{//<<-------------------------------------------------------------- Method save
            ...
            //Bandera de ejecución de consultas SQL
            $do = false;

            //Bandera que indica si se realizará un INSERT o un UPDATE
			$insert = ( $this->_pk!=0 ) ? false : true;
            
            //Acción a realizar dependiendo del valor de la llave primaria
			...
			if( $do === false ) {//<<---------------- If !do
                ...
                //Obtener el id recién ingresado si se realizó un INSERT
                if( $insert === true ) {
    ...
    /*
     * Genera consultas UPDATE dinámicamente.
     * @access    private
     * @params    Array fields->Arreglo con los campos a editar
     *            boolean escape->Determina si se escaparán los contenidos de los campos
     ...
     */
        ...
        {//<<-------------------------------------------------------------- Method _getUpdateSQL
            ...
            foreach($fields as $index => $value){//<<---------------- Crea arreglo valores
                ...
                //Identificación de la llave primaria
    ...
    /*
     * Genera consultas INSERT dinámicamente.
     * @access    private
     * @params    Array fields->Arreglo con los campos a editar
     *            boolean escape->Determina si se escaparán los contenidos de los campos
     * @methods   classes.Escapa::escaparFuncion() 
	...
	/*
	 * Obtiene el ID del último registro insertado en la tabla.
	 * @access	public
	...
	/*
	 * Borra el registro correspondiete al Id introducido como parámetro
	 * @access	public
	 ...
	 */
		...
		/***
	    *   @description:  Método que permite iniciar una transacción
		*   @param:        void
		*   @return:       boolean
		*   @updater:      TH
	    ...
	    ***/
		...
		/***
	    *   @description:  Método que hece rollback a una transacción
		*   @param:        void
		*   @return:       boolean
		*   @updater:      TH
		*   @updated_date: 25/08/2010
	    ***/
		...
		/***
	    *   @description:  Método que ejecuta la transacción
		*   @param:        void
		*   @return:       boolean
		*   @updater:      TH
		*   @updated_date: 25/08/2010
     ...
     */
     ...
     /*
      *  Destructor General
      *  Liberar Recursos y desconexión con la Base de Datos
      *  @access public
      */        
